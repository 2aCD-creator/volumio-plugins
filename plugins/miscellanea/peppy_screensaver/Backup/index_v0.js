      ...
      self.updateALSAConfigFile().then (function(){
        ...
        if (alsaconf < 2) {
            self.get_output_enabled(MPD).then (function(OutEnabled) {
                if (OutEnabled) {
                    self.switch_alsaConfig(MPD, alsaconf);                      
                }  
            });
...
peppyScreensaver.prototype.getUIConfig = function() {
    ...
    self.commandRouter.i18nJson(__dirname+'/i18n/strings_'+lang_code+'.json',
        ...
        if (fs.existsSync(config_file)){
            ...
            // screen width/height
            uiconf.sections[0].content[3].value = self.config.get('screenWidth');
            uiconf.sections[0].content[4].value = self.config.get('screenHeight');
            for (var i = 1; i < 3 ; i++) {
            ...
            if (fs.existsSync(meters_file)){
                ...
                if ((config.current.meter).includes(',')) {
                    uiconf.sections[0].content[6].value.value = 'list';
                } else {
                    uiconf.sections[0].content[6].value.value = config.current.meter;
                }
                uiconf.sections[0].content[6].value.label = (uiconf.sections[0].content[6].value.value).replace(upperc, c => c.toUpperCase());
                ...
                uiconf.sections[0].content[7].value = self.config.get('randomSelection');
                uiconf.sections[0].content[7].doc = self.commandRouter.getI18nString('PEPPY_SCREENSAVER.RANDOMSELECTION_DOC') + '<b>' + availMeters + '</b>';

                // random interval / smooth buffer
                uiconf.sections[0].content[8].value = parseInt(config.current['random.meter.interval'], 10);
                uiconf.sections[0].content[9].value = parseInt(config.data.source['smooth.buffer.size'], 10);                                
                for (var i = 3; i < 5 ; i++) {
                  minmax[i] = [uiconf.sections[0].content[i+5].attributes[2].min,
                    uiconf.sections[0].content[i+5].attributes[3].max,
                    uiconf.sections[0].content[i+5].attributes[0].placeholder];
                }
            }
            
            // mouse support
            var mouseSupport = (config.sdl.env['mouse.enabled']).toLowerCase() == 'true' ? true : false;
            uiconf.sections[0].content[10].value = needleCache;
...
// switch alsa config
peppyScreensaver.prototype.switch_alsaConfig = function (data, alsaConf) {
    const self = this;
    var defer = libQ.defer();
    var enableDSD = alsaConf == 2 ? true : false;
...
peppyScreensaver.prototype.MPD_setOutput = function (data, enableDSD) {
  ...
  exec(sedStr +  data, { uid: 1000, gid: 1000 }, function (error, stdout, stderr) {
    if (error) {
        ...
        defer.resolve();
        //self.logger.info('________ set to yes');
    }
  });
...
// enable the MPD allowed_formats
        ...
        if (!enableDSD) {
            exec("sed -i '/${special_settings}/a allowed_formats\t\x22192000:*:* 96000:*:* 88200:*:* 48000:*:* 44100:*:* 32000:*:* 16000:*:*\x22' " + data, { uid: 1000, gid: 1000 }, function (error, stdout, stderr) {
                if (error) {
                    ...
                    defer.resolve();
                //self.logger.info('________ set to no');
                }
            });
        }    
    }
...
#<--- end peppymeter"';
  ...
  try {  
    execSync("awk 'NR==FNR{if ($0 ~ /multiroom/){c=NR};next} {if (FNR==(c-4)) {print " + insertStr + " }};1' " +  data + " " + data + " > " + data_tmp + " && mv " + data_tmp + " " + data, { uid: 1000, gid: 1000 });
  } catch (error){
...
peppyScreensaver.prototype.writeAsoundConfigModular = function (alsaConf) {
  ...
  if (fs.existsSync(asoundTempl)) {
    ...
    if (alsaConf == 2) {
        ...
        conf = conf.replace('${alsaDuplicate}', 'peppyalsa');
    } else {
        conf = asounddata.replace('${alsaDirect}', 'peppyalsa');
        conf = conf.replace('${alsaDuplicate}', 'Peppyalsa');
    }
    
    var rate = alsaConf == 0 ? '44100' : '16000';
