...
from configfileparser import METER, BASE_PATH, SCREEN_INFO 
    ...
    def run(self):
        ...
        def on_push_state(*args):
            ...
            if args[0]['status'] == 'play':
                ...
                if (self.meter_config_volumio[METER] != 'random' and self.meter_config_volumio[METER] != 'list') or self.meter_config_volumio[RANDOM_TITLE] != True or self.first_run == True:
...
class ImageTitleFactory():
    ...
    def __init__(self, util, base, meter_config_volumio):
        ...
        self.meter_section = meter_config_volumio[self.util.meter_config[METER]]
        self.base = base
        self.titleMem = ""
    ...
    def render_aa(self, firstrun):
        ...
        if self.meter_section[ALBUMART_POS]:
            ...
            if self.meter_section[ALBUMART_MSK]:
                #mask_image = self.base.load_image(self.meter_section[ALBUMART_MSK])
                #self.screen.blit(mask_image[1], aa_rect) # draw albumart mask
                try:
                    # load mask image
                    base_path = self.util.meter_config[BASE_PATH]
                    folder = self.util.meter_config[SCREEN_INFO][METER_FOLDER]
                    path = os.path.join(base_path, folder, self.meter_section[ALBUMART_MSK])
                    mask_image = Image.open(path)                    
                    # get albumart form surface
                    raw_str = pg.image.tostring(self.aa_img, 'RGBA', False)
                    aa_image = Image.frombytes('RGBA', self.aa_img.get_size(), raw_str)
                    # scale mask if needed
                    if mask_image.size != aa_image.size:
                        mask_image = mask_image.resize(aa_image.size)
                    # convert to gray if needed
                    if mask_image.mode != 'L':
                        mask_image = ImageOps.grayscale(mask_image)
                    # image composite
                    empty_image = Image.new('RGBA', aa_image.size, 0)
                    compo_image = Image.composite(empty_image, aa_image, mask_image)
                    py_image = pg.image.fromstring(compo_image.tobytes(), compo_image.size, compo_image.mode)
                    # draw mask albumart
                    self.screen.blit(py_image, aa_rect)
                except:
            ...
            if self.meter_section[ALBUMBORDER]:
                pg.draw.rect(self.screen, self.fontcolor, aa_rect, self.meter_section[ALBUMBORDER])
                
            # update albumart rectangle
            #self.base.update_rectangle(aa_rect)
            pg.display.update(aa_rect)
    ...
    def render_text(self, firstrun):
        ...
        if self.titleMem != self.playinfo_title: # only if title changed
            ...
            if self.meter_section[PLAY_TITLE_POS] and (self.meter_section[PLAY_MAX] or self.meter_section[PLAY_TITLE_MAX]):
                title_rect = pg.Rect(self.meter_section[PLAY_TITLE_POS], (set_txt_max(self.meter_section[PLAY_TITLE_MAX]), imgTitle_long.get_height()))
            ...
            if self.meter_section[PLAY_ARTIST_POS] and (self.meter_section[PLAY_MAX] or self.meter_section[PLAY_ARTIST_MAX]):		
                artist_rect = pg.Rect(self.meter_section[PLAY_ARTIST_POS], (set_txt_max(self.meter_section[PLAY_ARTIST_MAX]), imgArtist_long.get_height()))
                if firstrun: # backup clean area on first run
            ...
            if self.meter_section[PLAY_ALBUM_POS] and (self.meter_section[PLAY_MAX] or self.meter_section[PLAY_ALBUM_MAX]):
                album_rect = pg.Rect(self.meter_section[PLAY_ALBUM_POS], (set_txt_max(self.meter_section[PLAY_ALBUM_MAX]), imgAlbum_long.get_height()))
                if firstrun: # backup clean area on first run
